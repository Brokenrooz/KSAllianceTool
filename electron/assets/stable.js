...
function migrateAlliance(a) {
  ...
  if (typeof a.useAlias !== "boolean") a.useAlias = false;
  return a;
}
...
function openMemberModal(mode) {
  ...
  if (useAlias) els.aliasBlock.classList.remove("hidden");
  else els.aliasBlock.classList.add("hidden");
...
function openAllianceModal(mode) {
  ...
  if (mode === "add") {
    ...
    state.allianceEditOriginalTag = null;
  } else {
    const a = getCurrentAlliance();
    if (!a) return;
    ...
    els.aUseAlias.checked = !!a.useAlias;
...
async function saveAllianceFromModal() {
  ...
  const useAlias = !!els.aUseAlias.checked;
  ...
  if (state.allianceModalMode === "add") {
    ...
    data.alliances.push({ id: tag, tag, name, info, useAlias, members: [] });
    await saveData();
    closeAllianceModal();
    selectAlliance(tag);
    return;
...
async function saveAddUserFromModal() {
  ...
  if (!tcParsed.rank) {
    alert("Level must be TC 1–30 or TG1–TG5.");
    els.auTC.focus();
    return;
  }

  const tag = (els.auAlliance.value ?? "").trim(); // optional
  const alias = (els.auAlias.value ?? "").trim();
  const notes = (els.auNotes.value ?? "").trim();

  const powerParsed = parsePowerInput(els.auPower.value);
  const mystic = normalizeNum(els.auMystic.value);
  ...
  data.players[gid] = {
    ...
    alias,
    tc: tcParsed.label,
    power: powerParsed ?? (existing.power ?? 0),
  ...
  if (els.updateDeltaBox) {
    ...
    els.updateDeltaBox.textContent = [
      `Player: ${name}`,
      `Alliance: ${showTag}`,
      `Power: ${formatPower(normalizeNum(data.players[gid].power))}`,
      `Mystic: ${normalizeNum(data.players[gid].mystic)}`,
    ].join("\\n");
  }
